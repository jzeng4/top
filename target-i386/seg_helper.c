...
void finish_tracing(void)
{
	print_program();
	gettimeofday(&tim, NULL);
	g_t2 = tim.tv_sec+(tim.tv_usec/1000000.0);
	fprintf(stdout, "@time\t%.12lf seconds elapsed\n", g_t2-g_t1);
}
...
//jzeng
	if(intno == 0x80){
		if(PEMU_cr3 == PEMU_get_cr3(0)){
			if(PEMU_get_reg(XED_REG_EAX) == 252){//exit_group
...
//jzeng
	if(PEMU_cr3 == PEMU_get_cr3(0)){
		if(PEMU_get_reg(XED_REG_EAX) == 94){//exit_group
//#ifdef WINDOWS_FORMAT
#ifdef DEBUG
...
//jzeng
#include "rev_inline/linux.h"
#include "config.h"
uint32_t start_trace = 0;
extern int PEMU_find_mmap(uint32_t nextaddr);

void helper_inst_hook(int pc1)
{
	seg_det=pc1;
	if(PEMU_start == 1)
	{
		uint32_t pc = (uint32_t)pc1;
		if(pc == hookingpoint)//flush_signal_handlers
		{
			PEMU_find_process(0);
		}
		...
		if(PEMU_cr3 == PEMU_get_cr3(0))
		{
			...
			PEMU_find_mmap(PEMU_task_addr);
			find_main_function(pc);
			
		//TODO
		//Hush.b
		//Let's trace into library, just propagate the taint, but doesn't generate new taint source
			if(pc < PEMU_txt_start || pc > PEMU_txt_end)
				return;
		//Hush.e	

			if(!g_pc){ //the first pc of text, recorded in start_addr file
				g_start_pc = pc;
				load_start_addr();
#ifdef DEBUG
				fprintf(stdout, "_start:\t%x\n", g_start_pc);
#endif
			}
				
			g_pc = pc;	
			if(pc == PEMU_main_start){
				gettimeofday(&tim, NULL);
				g_t1 = tim.tv_sec+(tim.tv_usec/1000000.0);

				init_top(PEMU_main_start);
				start_trace = 1;
			}
			if(start_trace){
				PEMU_read_mem(pc, 15, g_inst_buffer);
				Instrument_PC1(g_inst_buffer);
			}
